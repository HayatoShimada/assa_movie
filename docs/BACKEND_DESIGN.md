...
| 形式 | 出力例 | 特徴 |
|---|---|---|
| 注釈(既定) | それ(先月のイベント)がすごくて | **発言を一切改変しない**ため最も安全。字幕の標準的な作法。confidence=autoで即適用可 |
| 置換 | 先月のイベントがすごくて | 読みやすいが発言の改変になる。現行プロトタイプの動作 |
| 補完 | それ(先月のイベント)が(運営も内容も)すごくて | 言い落とされたニュアンスまで括弧で補う意訳寄りの形式。**全件review必須**(自動適用禁止) |
