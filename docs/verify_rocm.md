...
> **2026-08-10 追記:** この文書は当時の実測記録として残している。現在は
> ROCm/HIPもtorchも使わない。AMD GPUでも同梱のwhisper.cpp(Vulkan)で動き、
> エンジンは実行環境プロファイルで固定される(DESIGN.md 2026-08-10)。
> 手順のうちtorch・pyannote・transformers・HIPビルドに関する箇所は現行版に
> 当てはまらない。**Vulkan と HIP の速度比較(下記)だけが今も有効な記録。**
...
(2026-08-10追記: 以下は当時の記録。現在はHIPビルドを使わずVulkanに統一し、
エンジンは実行環境プロファイルで固定する。DESIGN.md 2026-08-10 を参照)
当時のROCmのエンジン自動選択がこれを使った。用意していない環境では公式Whisperに
