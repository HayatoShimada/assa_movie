...
**保存場所**: 設定ディレクトリのファイル(パーミッション0600)。
当初はOSの資格情報ストア(Tauri keyringプラグイン)を予定していたが、
**採用しなかった**。3OSぶんのキーチェーン差異を抱えるより、ローカル専用アプリの
実態(同じマシンの他プロセスから読めるかどうかは資格情報ストアでも大差ない)に
合わせた。APIキーも同じ扱い(`backend/api/keys_api.py`)。
...
- Windows: MSI/NSIS。**whisper.cpp の Vulkan バックエンドで統一する**方針
  ...
  HIP比13%遅いだけで、ROCmを一切リンクせず44MB(静的なら1ファイル43MB)。
  AMD/NVIDIA/Intel を1ビルドで賄えるため、GPUごとにビルドを分けずに済む。
  この大きさならアプリに同梱でき、M28の「ビルドが要るので入れられない」制限も消える。
...
- ~~初回起動時のモデル取得の進捗UI~~ ✅ 設定タブに「セットアップ」パネルを追加。
  何が足りていないかを示し、その場でダウンロードできる
  (既存のジョブキュー+SSEに乗せたので進捗バーが出る)
- ~~whisper.cppはビルドが要るのでアプリからは入れられない~~ ✅ Vulkanビルド(43MB)を
  同梱するようにした。ユーザーはモデル(3.1GB)を画面から落とすだけで使える
...
### M30〜M38: Windowsで実際に動くようにする(v0.9.2〜v0.9.8)

3OSぶんのインストーラは出来ていたが、**Windows版は起動すらしなかった**。
以降は「入れて・起動して・使えるところまで」を毎回実機で確かめている。

| # | 内容 |
|---|---|
| M30 | 同梱したwhisper.cppを見つける(`.exe` を付けずに探していて見つからなかった) |
| M31 | Linux以外のOSの置き場所とエンコーダ選択(OS判定を注入して純関数で確かめる) |
| M32 | Windowsで起動する。サイドカー名の `.exe` 欠落、cp932でのprint、Job Objectでの後始末 |
| M33 | 初回セットアップのウィザード(環境検出・エンジン選択・APIキー登録) |
| M34 | GPUがあるのに「GPUなし」と出る(WindowsではCLIが無いのでレジストリから読む) |
| M35 | 話者分離モデルとwhisper.cppを配布物に同梱する |
| M36 | 子プロセスの出力をロケール依存で復号しない |
| M37 | ジョブの中止と、プロジェクト削除時のプロセス停止 |
| M38 | セットアップが固定したASRエンジンを一度だけ解除する(autoに戻す) |

### M39〜M41: 戦略的リファクタリング(2026-08-09)

v0.9.3〜v0.9.8で**同じ型の失敗を3種類・計10回**繰り返した。
原因は「CIにテスト実行ジョブが存在しない」ことだったので、そこから直した。

| 失敗の型 | 回数 |
|---|---|
| ローカルで確認した経路とCIの経路が違う | 3 |
| Windowsのロケール依存エンコーディング | 5 |
| ビルドは通るが実際には動かない | 3 |

| # | 内容 |
|---|---|
| M39 | 配布中のまま残っていたバグ7件(ROCmのフォールバック先・ジョブ状態名の乖離・cp932のAPIキー・非ASCIIパスの顔検出・Windowsのフォント列挙・AppImageのffmpeg案内) |
| M40 | 効いていない設定を消す(UIに出ているのにバックエンドが読んでいなかった5項目) |
| M41 | torch依存の2エンジン(transformers版Whisper / pyannote)を削除。配布版では一度も動かない選択肢だった |

あわせて行ったこと:

- **`.github/workflows/ci.yml` を新設**(3OSのpytest / フロント / cargo test / E2E)。
  以降は「手元で流したか」ではなくCIの緑をもってマージ可能とする
- Windowsで `uv sync` が失敗する原因を解消(ROCmのwheelはLinuxにしか無い)
- E2Eが8001番の**古いサーバを使い回していた**。バックエンドを直しても古いコードに
  対してテストが通る状態だったので `reuseExistingServer` を切った
- ライセンス条件の充足(whisper.cppの表記が`.ps1`にしか無く、`.deb`/`.AppImage`/`.dmg`
  はMITの再配布条件を満たしていなかった。Apache-2.0の本文も未同梱だった)
- `.deb` / `.AppImage` / `.dmg` の実機検証(`scripts/verify_unix.sh`)。
  この3つは一度もインストール・起動されずに配布されていた
- テストを `tests/{core,engines,pipeline,api}/` へ再編し、重複していた下準備と
  字幕のケース表(Python 25件 / TS 15件と乖離していた)を共有した
