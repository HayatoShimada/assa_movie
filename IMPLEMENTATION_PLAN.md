...
**全マイルストーン完了(2026-08-07)。** テスト: backend 243件 / frontend unit 28件 / E2E 18件。

未実装として残っているもの(README「既知の制限」参照):
- 8-6 縦型レイアウト・顔追従クロップ(要検証マークのまま)
- 字幕スタイルテンプレートの保存UI・複数クリップの一括書き出しUI
- whisper.cpp / ROCm / Windows(Phase 3)
